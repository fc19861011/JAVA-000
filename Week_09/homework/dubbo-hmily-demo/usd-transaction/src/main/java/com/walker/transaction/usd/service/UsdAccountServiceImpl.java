...
public class UsdAccountServiceImpl implements UsdAccountService  {
    ...
    public boolean usdPayment(Integer payerId, Integer count, Long transId) throws Exception {
        UsdAccount usdAccount = usdAccountRepository.getOne(payerId);
        Integer amount = usdAccount.getAmount();
        if (amount < count) {
            throw new Exception("余额不足");
        }
        UsdAccountFreeze accountFreeze = usdAccountFreezeRepository.findByUserIdAndFreezeType(payerId, transId);
        int freezeCount = 0;
        if (null == accountFreeze) {
            ...
            freezeCount = usdAccountFreezeRepository.accountFreeze(payerId, count, transId);
        }
    ...
    @Transactional(rollbackFor = Exception.class)
    public void confirmUsdPayment(Integer payerId, Integer count, Long transId) {
        System.out.println("【" + payerId + "】支付 ($" + count + ") 成功");
        usdAccountFreezeRepository.accountFinish(payerId, count, transId);
    }

    @Transactional(rollbackFor = Exception.class)
    public void cancelUsdPayment(Integer payerId, Integer count, Long transId) {
        System.out.println("【" + payerId + "】支付 ($" + count + ") cancel");
        UsdAccountFreeze accountFreeze = usdAccountFreezeRepository.findByUserIdAndFreezeType(payerId, transId);
        if(null == accountFreeze || accountFreeze.getAmount() == 0) {
            return;
        }
    ...
    public boolean usdCollection(Integer payerId, Integer count, Long transId) throws Exception {
        UsdAccountFreeze accountFreeze = usdAccountFreezeRepository.findByUserIdAndFreezeType(payerId, transId);
        int freezeCount = 0;
        if (null == accountFreeze) {
            accountFreeze = new UsdAccountFreeze();
    ...
    @Transactional(rollbackFor = Exception.class)
    public void confirmUsdCollection(Integer payerId, Integer count, Long transId) {
    ...
    @Transactional(rollbackFor = Exception.class)
    public void cancelUsdCollection(Integer payerId, Integer count, Long transId) {
